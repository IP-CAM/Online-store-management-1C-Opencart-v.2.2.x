   ...
   */
	...
	*/
	public function sendProducts($args, $signature) {
		...
		$lang = $this->_getLanguageId('ru-ru');
	...
	private function _saveProductOptions($product_id, $args, $option_exchange) {
		...
		$language_id = $this->_getLanguageId('ru-ru');
		$languages = $this->_getLanguageId();
		$sort_order = 1;
		$required = 1;
		$type = 'select';
	...
	*/
	private function _getAllCatsForProd($category_id) {
		...
		$category_description = $this->model_catalog_category->getCategoryDescriptions($category_id);
		$category_info = array_merge(isset($category_description[$this->_getLanguageId('ru-ru')]) ? $category_description[$this->_getLanguageId('ru-ru')] : array(), $this->model_catalog_category->getCategory($category_id));
		...
		if (isset($category['parent_id']))
		{
			while ($category['parent_id'] > 0)
			{
				...
				$category = array_merge($category_description[$this->_getLanguageId('ru-ru')], $this->model_catalog_category->getCategory($category['parent_id']));
	...
	*/
	private function _getAllCatsForCat($category_info) {
		...
		if (isset($category['parent_id']))
		{
			...
			{
				$category_description = $this->model_catalog_category->getCategoryDescriptions($category['parent_id']);
				$lang1 = $this->_getLanguageId('ru-ru');
				if (!empty($lang1)){
					$category = array_merge($category_description[$this->_getLanguageId('ru-ru')], $this->model_catalog_category->getCategory($category['parent_id']));
				} else {
